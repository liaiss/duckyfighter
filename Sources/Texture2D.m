...
- (id) initWithRect:(NSInteger) rectWidth :(NSInteger) rectHeight:(CGFloat) red: (CGFloat)green:(CGFloat) blue: (CGFloat) alpha {
	...
	CGFloat fillColor[] = {red, green, blue, alpha};
	
    pixelFormat = kTexture2DPixelFormat_RGBA8888; 
	
	imageSize = CGSizeMake(rectWidth, rectHeight);
	// transform = CGAffineTransformIdentity;
...
- (void) drawAtPoint:(NSInteger) x: (NSInteger) y {
	...
	//TODO: This vertices value is a workaround, we invert x and y component to adapt texture to landscape view.
	GLfloat		vertices[] = {	y,			x,	0.0f,
		                        y,	width + x,	0.0f,
		                        height  + y, x,				0.0f,
                                height  + y,width + x,	0.0f };
